...
async function loadPropertiesTable() {
    ...
    try {
        ...
        const session = await getCurrentUserWithRefresh();//await supabase.auth.getSession();
        let us = session.data.email; //session.data.session.user.email
        ...
        //const { data, error } = await supabase.from('main_parcels').select('*').eq('owner_uuid', us);
    ...
    data.forEach((row) => {
        ...
        columns.forEach(col => {
            ...
            if (col === 'sell_category' || col === 'status_category') {
                ...
                // Regular text/number input (unchanged)
