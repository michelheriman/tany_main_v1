      ...
      if (element.sender_id === user) {
        ...
        <div class="flex items-end justify-end gap-3 px-4 sm:px-6 md:px-12 py-4">
          <div class="bg-gradient-to-br from-green-600 to-emerald-500 text-white p-5 sm:p-6 rounded-2xl shadow-xl max-w-md w-full transition-transform duration-300 hover:scale-[1.02]">
            ...
            <p class="text-sm sm:text-base leading-relaxed">${element.content}</p>
            <span class="text-xs text-green-100 block mt-3 text-right">${element.created_at}</span>
          </div>
        </div>
       ...
       let mes = `
        <div class="flex items-start gap-3">
            <div class="bg-blue-500 text-white p-4 rounded-xl shadow max-w-md">
                <p class="text-sm sm:text-base leading-relaxed font-black italic tracking-wider text-blue-100 drop-shadow-sm mb-2">${element.sender_id}</p>
                <p>${element.content}</p>
                <span class="text-xs text-blue-200 block mt-2 text-right">${element.created_at}</span>
                </div>
        </div>
       `
       ...
       let mess = `
        <div class="flex items-start gap-3">
            <div class="bg-blue-500 p-4 rounded-xl shadow max-w-md">
                <h4 class="text-xl sm:text-2xl md:text-3xl font-extrabold italic text-white tracking-wide">${element.sender_id}</h3>
                <p> ${element.content}</p>
          ...
          <button onclick = "load('${objt}', '${element.uuid_post}')" class="w-full text-left flex items-center gap-4 p-4 hover:bg-gray-100 transition rounded-lg">
              ...
              <div class="flex-1">
                <div class="flex justify-between">
                  <h4 class="font-semibold text-gray-800">${mmail}</h4>
                  
                </div>
                <p class="text-sm text-gray-600 truncate">${objt}</p>
              </div>
        </button>
